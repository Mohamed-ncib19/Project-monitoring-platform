...
const Step1Form = ({
  ...
  const getProjectParents = async (id) => {
    try {
      ...
      const portfolioParent = portfolios?.find(
        (portfolio) => response?.data.product.portfolio === portfolio?.value,
      );
      if (portfolioParent) {
        setDefaultPortfolioParent(portfolioParent);
        setDefaultProductParent({
          value: response?.data.product._id,
  ...
  useEffect(() => {
      ...
      resetForm({
        portfolio : defaultPortfolioParent?.value,
        product :defaultProductParent?.value,
      });
  
  }, [resetForm, defaultPortfolioParent, defaultProductParent]);
    ...
    const fetchData = async (portfolio) => {
      try {
        ...
        if (
          ...
          const transformedProducts = await productsData.data.products.map(
            (product) => ({ value: product?._id, label: product?.name }),
          );
          setProductOption(transformedProducts);
        } else {
          notify({
            message: `There are no Products for ${portfolio?.label} `,
            status: 'warning',
          });
          setProductOption([]);
        }
        ...
        setProductOption([]);
        notify({
          message: `${JSON.parse(error?.request?.response).message} for ${selectedPortfolio ? selectedPortfolio?.label : ''} `,
          status: 'warning',
        });
      }
    };
    ...
    <>
      <div className="d-flex flex-lg-row flex-column justify-content-lg-around justify-content-center col-lg-10 col-12 gap-4 align-items-center">
        ...
        <div className="col-lg-8 col-12 z-index-1000">
          <Controller
            ...
            render={({ field }) => (
              <Select
                ...
                onBlur={field.onBlur}
                isDisabled={!projectRootLayer}
                value={
                  !projectRootLayer
                    ? defaultPortfolioParent || ''
                    : portfolios.find(
          ...
          />
          {errors.portfolio && (
            <span className="text-danger">{errors.portfolio.message}</span>
          )}
        </div>
      </div>
      ...
      <div className="d-flex flex-lg-row flex-column justify-content-lg-around justify-content-center col-lg-10 col-12 gap-4 align-items-center">
        <label htmlFor="prodcode" className="text-danger">
          Product Code
        </label>
      ...
      <div className="d-flex flex-lg-row flex-column justify-content-lg-around justify-content-center col-lg-10 col-12 gap-4 align-items-center">
        <label htmlFor="date" className="text-muted text-end ">
...
const Step2Form = ({ control, notify, setValue }) => {
...
export const ProjectHeader = ({
  ...
  const LabelIconsArray = [<DetailsIcon />, <MembersIcon />, <FrameworkIcon />];

  const getCurrentSchema = (step) => {
    switch (step) {
      case 1:
        return ProjectSchema.Step1Schema;
      case 2:
        return ProjectSchema.Step2Schema;
      case 3:
        return ProjectSchema.Step3Schema;
      default:
        return ProjectSchema.Step1Schema;
    }
  };
          ...
          <form className="d-flex flex-column gap-5 py-5">
            {currentStep === 1 && (
              <Step1Form
                ...
                productId={productId}
                projectRootLayer={projectRootLayer}
              />
            )}
            {currentStep === 2 && (
              <Step2Form
