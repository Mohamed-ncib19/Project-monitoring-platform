...
const SetupEditForm = ({ user }) => {
  ...
  const onSubmit = async (data) => {
    try {
      const { firstname, lastname, bio, phone, email, position, salary, role } = data;
      const cleanData = {};

      if (firstname && firstname !== user.firstname) cleanData.firstname = firstname;
  ...
  return (
    <div>
      <FormProvider {...methods}>
        <form className="d-flex flex-column gap-4 py-5" onSubmit={handleSubmit(onSubmit)}>
