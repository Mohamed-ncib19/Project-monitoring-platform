...
const Table = ({ columns, data, defaultPageSize = 10, pageSizeOptions = [10, 20, 30] }) => {
  ...
  for (let i = 0; i < pageCount; i++) {
    pageButtons.push(
      <button key={i} onClick={() => gotoPage(i)} className={`page-button ${pageIndex === i ? 'active' : ''}`}>
        {i + 1}
      </button>
    );
  }
      ...
      <div className="pagination bg-white d-flex justify-content-between py-4">
        <div>
          <span className="mx-2 light-text-custom-color ">
            Showing{' '}
            <strong>
              {pageIndex * pageSize + 1} to {Math.min((pageIndex + 1) * pageSize, data.length)} of {data.length}
            </strong>{' '}
            entries
          </span>
        </div>
        ...
        <div className="d-flex align-items-center gap-5">
          <div className="d-flex align-items-center">
            <span className='light-text-custom-color' >
              ...
              <input
                ...
                onChange={(e) => {
                  const items = e.target.value ? Number(e.target.value) : pageSizeOptions[0];
          ...
          <div className="d-flex align-items-center px-3">
            <button className="prev-button rounded-1 border-0" onClick={() => previousPage()} disabled={!canPreviousPage}>
            ...
            <button className="next-button rounded-1 border-0" onClick={() => nextPage()} disabled={!canNextPage}>
