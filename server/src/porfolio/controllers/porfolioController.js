...
const porfolioServices = require("../services/porfolioServices");
const porfolioController = {
  async createPorfolio(request, reply) {
    try {
      const porfolioData = request.body;
      const response = await porfolioServices.porfolioExists(porfolioData.name);
      if (!response.exists) {
        const response = await porfolioServices.createporfolio(porfolioData);
        return reply
